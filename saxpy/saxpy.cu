...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double startTime_kernal = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();
    double endTime_kernal = CycleTimer::currentSeconds();
    //
    ...
    double overallDuration_kernal = endTime_kernal - startTime_kernal;
    printf("Effective BW by CUDA kernal: %.3f ms\n", 1000.f * overallDuration_kernal);
